...
function SuperAdminDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, pending: 0, active: 0, pendingAppts: 0, todayAppts: 0, weekAppts: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const confirmed = appts.filter((a) => a.status === "confirmed" && a.date && a.date.toDate() >= now);
      ...
      setStats({
        ...
        todayAppts:   appts.filter((a) => a.status === "confirmed" && fmtDateIso(a.date) === todayStr).length,
        weekAppts:    appts.filter((a) => a.status === "confirmed" && a.date && a.date.toDate() >= now && a.date.toDate() <= weekEnd).length,
      });
      setPendingMembers(users.filter((u) => u.status === "pending"));
      setNextAppts(confirmed.slice(0, 3));
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: "Pending Appts",    value: stats.pendingAppts, icon: Calendar, color: "bg-purple-50 text-purple-600" },
            ].map((s) => (
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#F3F4F6]">
              <h2 className="text-[#1B2E6B] font-bold text-sm">Appointments Overview</h2>
            ...
            <div className="grid grid-cols-3 divide-x divide-[#F3F4F6]">
              {[
                { label: "Today",         value: stats.todayAppts,   icon: "📅" },
                { label: "This Week",     value: stats.weekAppts,    icon: "📆" },
                { label: "Pending Review", value: stats.pendingAppts, icon: "⏳" },
              ].map((s) => (
            ...
            {nextAppts.length > 0 && (
              <div className="border-t border-[#F3F4F6]">
                <p className="text-xs font-semibold text-[#9CA3AF] px-5 py-3">NEXT 3 UPCOMING</p>
                  ...
                  {nextAppts.map((a) => (
                    <div key={a.id} className="px-5 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[#374151] text-xs font-medium">{a.memberName ?? "—"}</p>
                          <p className="text-[#9CA3AF] text-[10px] capitalize">{a.serviceType}</p>
                        </div>
                      </div>
                      <p className="text-[#1B2E6B] text-xs font-semibold">{a.time || fmtDate(a.date)}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    {promoting === m.id ? (
                      ...
                      <>
                        <button onClick={() => setPromoting(m.id)}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        ...
        {loadingAppts ? (
          ...
          <div className="py-8 text-center text-[#9CA3AF] text-sm">No upcoming appointments</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#F9FAFB] border-b border-[#F3F4F6]">
                  {["Member", "Service", "Date", "Time"].map((h) => (
                  ...
                  <tr key={a.id} className="hover:bg-[#F9FAFB]">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-6 h-6 rounded-full bg-[#EEF1F8] flex items-center justify-center text-[9px] font-bold text-[#1B2E6B] flex-shrink-0">
                          {a.memberName?.[0]?.toUpperCase() ?? "?"}
                        </div>
...
function MemberDashboard() {
  ...
  useEffect(() => {
    ...
    getDocs(
      ...
      const confirmed = appts.find((a) => a.status === "confirmed" && a.date && a.date.toDate() >= now) ?? null;
      const pendingN  = appts.filter((a) => a.status === "pending").length;
      setNextConfirmed(confirmed);
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#F3F4F6]">
          <div className="flex items-center gap-2">
            <h2 className="text-[#1B2E6B] font-bold text-sm">Your Next Appointment</h2>
            {pendingCount > 0 && (
          ...
          <div className="p-5">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-2xl bg-[#EEF1F8] flex items-center justify-center text-2xl flex-shrink-0">
                {{ confession: "🕊️", counseling: "🙏", marriage: "💍", other: "📋" }[nextConfirmed.serviceType] ?? "📋"}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <p className="text-[#1B2E6B] font-semibold text-sm capitalize">{nextConfirmed.serviceType}</p>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-50 text-green-700">CONFIRMED</span>
                </div>
                <p className="text-[#6B7280] text-xs">{fmtDate(nextConfirmed.date)} · {nextConfirmed.time || "—"}</p>
                {(() => {
                  const cd = countdown(nextConfirmed.date, nextConfirmed.time);
                  return cd ? <p className="text-[#1B2E6B] text-xs font-semibold mt-0.5">{cd}</p> : null;
                })()}
              </div>
            </div>
          </div>
